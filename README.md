# Hi, I'm Motsim Aslam 👋

### AI/ML · Natural Language Processing · Generative AI · Healthcare AI

I build intelligent, human-centered systems using **machine learning, natural language processing, generative AI, and multimodal technologies**, with a growing research focus on applications in **healthcare, education, and rehabilitation**.

I am particularly interested in how AI systems can transform complex real-world information into **interpretable, useful, and evidence-driven decisions**.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Professional_Profile-0A66C2?style=flat-square\&logo=linkedin\&logoColor=white)](YOUR_LINKEDIN)
[![Google Scholar](https://img.shields.io/badge/Google%20Scholar-Research-4285F4?style=flat-square\&logo=googlescholar\&logoColor=white)](YOUR_SCHOLAR)
[![ORCID](https://img.shields.io/badge/ORCID-Researcher_ID-A6CE39?style=flat-square\&logo=orcid\&logoColor=white)](YOUR_ORCID)
[![Portfolio](https://img.shields.io/badge/Portfolio-Academic-111827?style=flat-square\&logo=googlechrome\&logoColor=white)](YOUR_PORTFOLIO)

---

## 🔬 Research Interests

* **Natural Language Processing (NLP)**
* **Large Language Models & Generative AI**
* **Multimodal AI**
* **Healthcare & Clinical AI**
* **Human-Centered Artificial Intelligence**
* **Explainable & Trustworthy AI**
* **AI for Education**
* **AI applications in Rehabilitation & Movement Sciences**

---

## 🧪 Selected Research & AI Systems

### 🆘 CrisisGPT — AI Emergency Assistant

**NLP · LLMs · Multilingual AI · Disaster Response**

An AI-powered emergency intelligence system designed to classify, translate, summarize, and interpret disaster reports using **LLaMA 3.1**.

**Research / Engineering Focus**

* Multilingual incident understanding
* Automated classification and summarization
* LLM-based emergency response support
* Interactive geospatial visualization
* Human-centered information delivery

[→ View Project](https://github.com/MotsimAslam/CrisisGpt)

---

### 🏥 MedContextAI — Multi-Agent Medical Analysis

**Generative AI · Multi-Agent Systems · Healthcare AI · Document Intelligence**

A multi-agent medical analysis prototype that processes medical reports and coordinates multiple AI agents for extraction, evaluation, confidence assessment, and structured reporting.

**Architecture**

```text
Medical Document
       ↓
Information Extraction
       ↓
Clinical Evaluation
       ↓
Confidence Assessment
       ↓
Decision / Structured Report
```

**Research / Engineering Focus**

* LLM-powered document understanding
* Multi-agent coordination
* Medical information extraction
* Confidence-aware AI workflows
* Human oversight for high-stakes applications

[→ View Project](https://github.com/MotsimAslam/MedContextAI)

---

### 🌍 AfriLearn AI Tutor

**NLP · Multilingual AI · AI Education · Conversational Systems**

A multilingual AI tutoring platform designed to improve access to personalized education through conversational AI.

Supports:

* English, French, and Swahili interaction
* AI tutoring and conversational learning
* Voice-based interaction
* Career guidance
* Offline learning capabilities

**Research Question**

> How can multilingual and accessible AI systems reduce barriers to personalized education?

[→ View Project](https://github.com/MotsimAslam/Afri-AI-Tutor)

---

### ❤️ AI-Powered Cardiovascular Health Assistant

**Healthcare AI · Predictive Modeling · Generative AI · Multi-Agent Systems**

An AI-driven healthcare prototype combining cardiovascular risk prediction with personalized health guidance and intelligent patient-facing interfaces.

**Research / Engineering Focus**

* Predictive healthcare analytics
* AI-assisted patient decision support
* Multi-agent AI
* Health-data integration
* Personalized recommendations

[→ View Project](https://github.com/MotsimAslam/AI-Powered-Cardiovascular-Health-Assistant-Fall-in-Love-with-DeepSeek-Hackathon)

---

## 🧠 Research Themes

My work explores the intersection of:

```text
                 Artificial Intelligence
                         │
        ┌────────────────┼────────────────┐
        ↓                ↓                ↓
       NLP             LLMs        Multimodal AI
        │                │                │
        └────────────────┼────────────────┘
                         ↓
                  Human-Centered AI
                         │
          ┌──────────────┼──────────────┐
          ↓              ↓              ↓
     Healthcare       Education    Rehabilitation
```

The long-term goal is to develop AI systems that are not only capable, but also **interpretable, reliable, accessible, and meaningful in real-world human settings**.

---

## 🛠️ Technical Toolkit
## Tools & Libraries
[![MySQL Badge](https://img.shields.io/badge/MySQL-%23E74C3C.svg?style=for-the-badge&logo=mysql&logoColor=white)](https://www.mysql.com/)  [![Python Badge](https://img.shields.io/badge/Python-%2314354C.svg?style=for-the-badge&logo=python&logoColor=white)](https://www.python.org/)  [![scikit-learn Badge](https://img.shields.io/badge/scikit--learn-%23F7931E.svg?style=for-the-badge&logo=scikit-learn&logoColor=white)](https://scikit-learn.org/)  [![Pandas Badge](https://img.shields.io/badge/Pandas-%23150458.svg?style=for-the-badge&logo=pandas&logoColor=white)](https://pandas.pydata.org/)  [![Seaborn Badge](https://img.shields.io/badge/Seaborn-%230C55A5.svg?style=for-the-badge&logo=python&logoColor=white)](https://seaborn.pydata.org/) [![Matplotlib Badge](https://img.shields.io/badge/Matplotlib-%23FF9633.svg?style=for-the-badge&logo=python&logoColor=white)](https://matplotlib.org/)  [![Plotly Badge](https://img.shields.io/badge/Plotly-%233B4D98.svg?style=for-the-badge&logo=plotly&logoColor=white)](https://plotly.com/)  [![TensorFlow Badge](https://img.shields.io/badge/TensorFlow-%23FF6F00.svg?style=for-the-badge&logo=tensorflow&logoColor=white)](https://www.tensorflow.org/)  [![PyTorch Badge](https://img.shields.io/badge/PyTorch-%23EE4C2C.svg?style=for-the-badge&logo=pytorch&logoColor=white)](https://pytorch.org/) [![Keras Badge](https://img.shields.io/badge/Keras-%23D00000.svg?style=for-the-badge&logo=keras&logoColor=white)](https://keras.io/)  [![OpenCV Badge](https://img.shields.io/badge/OpenCV-%235C3EE8.svg?style=for-the-badge&logo=opencv&logoColor=white)](https://opencv.org/) [![Power BI Badge](https://img.shields.io/badge/Power%20BI-F2C811.svg?style=for-the-badge&logo=Power%20BI&logoColor=white)](https://powerbi.microsoft.com/) [![AWS Cloud Badge](https://img.shields.io/badge/AWS%20Cloud-FF9900.svg?style=for-the-badge&logo=Amazon%20AWS&logoColor=white)](https://aws.amazon.com/) [![Advanced Excel Badge](https://img.shields.io/badge/Advanced%20Excel-217346.svg?style=for-the-badge&logo=Microsoft%20Excel&logoColor=white)](https://support.microsoft.com/en-us/excel)

### AI / Machine Learning

Python · PyTorch · TensorFlow · Keras · Scikit-learn

### NLP / Generative AI

NLP · LLMs · Prompt Engineering · RAG · AI Agents · LlamaIndex

### Data & Scientific Computing

NumPy · Pandas · Matplotlib · Seaborn · Jupyter

### Application & Deployment

FastAPI · Streamlit · Docker · REST APIs · PostgreSQL

### Computer Vision

OpenCV · Image Processing · Deep Learning

---

## 📚 Research & Academic Work

### Current Research Direction

**AI + Healthcare + Human-Centered Rehabilitation**

I am interested in investigating how AI and multimodal technologies can support:

* clinical and rehabilitation assessment
* movement and posture analysis
* intelligent patient education
* personalized rehabilitation
* decision support for clinicians
* underserved healthcare populations

### Research Outputs

* 🔬 Research projects and experimental systems
* 📄 Publications and manuscripts
* 📊 Data analysis and visualization
* 🧪 Reproducible computational experiments

[Google Scholar](YOUR_SCHOLAR) · [ORCID](YOUR_ORCID)

---

## 🎓 Education

**PhD — Rehabilitation Sciences / Movement Sciences & Disorders**
Texas Tech University Health Sciences Center

**Doctor of Physical Therapy (DPT)**
Superior University Lahore

---

## 📈 What I Am Building Next

My current direction is moving from **AI application development toward research-driven AI systems**.

```text
Build
  ↓
Experiment
  ↓
Evaluate
  ↓
Measure
  ↓
Publish
  ↓
Translate into real-world impact
```

I am particularly interested in research collaborations involving **NLP, generative AI, healthcare AI, multimodal systems, and human-centered rehabilitation technologies**.

---

<div align="center">

### Research with purpose. AI with measurable impact.

[GitHub](https://github.com/MotsimAslam) · [LinkedIn](YOUR_LINKEDIN) · [Google Scholar](YOUR_SCHOLAR) · [ORCID](YOUR_ORCID)

</div>


## Social Profile
[![LinkedIn Badge](https://img.shields.io/badge/LinkedIn-0077B5.svg?style=for-the-badge&logo=LinkedIn&logoColor=white)](https://www.linkedin.com/in/your-link)
[![Medium Badge](https://img.shields.io/badge/Medium-12100E.svg?style=for-the-badge&logo=Medium&logoColor=white)](https://medium.com/@your-link)
[![Personal Website Badge](https://img.shields.io/badge/Personal%20Website-000000.svg?style=for-the-badge&logo=About.me&logoColor=white)](https://yourpersonalwebsite.com)
[![Kaggle Badge](https://img.shields.io/badge/Kaggle-20BEFF.svg?style=for-the-badge&logo=Kaggle&logoColor=white)](https://www.kaggle.com/motsimaslam)


## Contact
For collaborations or inquiries, reach out to me at [Click Here](mailto:mutasim.lion786@gmail.com)

<!---
MotsimAslam/MotsimAslam is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
